...
Run `npm install`, `npm run typecheck`, `npm run lint`, `npm test`, and `npm run build` in `plugin/`. The build writes `plugin/dist/manifest.json`, `plugin/dist/plugin.js`, and `plugin/dist/ui.html`; import the generated manifest in Figma Desktop as a development plugin and open it in each document. The UI persists its port and reconnects with bounded jittered backoff.
