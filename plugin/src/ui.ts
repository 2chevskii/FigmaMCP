...
let socket: WebSocket | undefined; let stopped = false; let reconnect = 0; let timer: number | undefined; const connectionId = crypto.randomUUID(); let configuredPort = 3846;
...
function context(): Record<string, unknown> { return { plugin_version: "0.1.0", editor_type: "figma", mode: "default", document_name: "Loading…", current_page: { id: "", name: "Loading…" } }; }
window.onmessage = event => { const message = event.data.pluginMessage as { type: string; port?: number; bytes?: Uint8Array }; if (message.type === "config_loaded" && message.port) { configuredPort = message.port; port.value = String(configuredPort); connect(); } else if (message.type === "bridge_frame" && message.bytes) { if (socket?.readyState === WebSocket.OPEN) socket.send(message.bytes); } else if (message.type === "context_dirty") { /* The next reconnect/metadata request is authoritative. */ } };
