...
function setState(value: State, message = ""): void {
...
function connect(): void {
  ...
  socket = new WebSocket(`ws://127.0.0.1:${configuredPort}/bridge`, "figma-mcp-bridge.v1");
  ...
  socket.onmessage = (event) => {
    if (typeof event.data === "string") {
      setState("protocol_error", "Bridge sent a text frame.");
      socket?.close();
      return;
    }
    try {
      ...
      receive(envelope, bytes);
    } catch {
      setState("protocol_error", "Invalid bridge message.");
      socket?.close();
    }
  };
  socket.onerror = () => {
    error.textContent = "Unable to connect to the companion server.";
  };
...
function receive(envelope: Envelope, bytes: Uint8Array): void {
  if (envelope.type === "hello_ack") {
    reconnect = 0;
    setState("connected");
    details.textContent = `Connection: ${connectionId}`;
    return;
  }
  if (envelope.type === "request") {
    post({ type: "bridge_frame", bytes });
    return;
  }
  if (envelope.type === "ping" && envelope.payload)
    send({
      type: "pong",
      protocol_version: PROTOCOL_VERSION,
      connection_id: connectionId,
...
function send(envelope: Envelope): void {
  if (socket?.readyState === WebSocket.OPEN) socket.send(pack(envelope));
}
function schedule(): void {
  if (stopped) return;
  const delays = [500, 1000, 2000, 5000, 10000];
  const base = delays[Math.min(reconnect++, delays.length - 1)];
...
window.onmessage = (event) => {
  ...
  if (message.type === "config_loaded" && message.port) {
    configuredPort = message.port;
    summary = message.context ?? summary;
    port.value = String(configuredPort);
    connect();
  } else if (message.type === "bridge_frame" && message.bytes) {
    if (socket?.readyState === WebSocket.OPEN) socket.send(message.bytes);
  } else if (message.type === "context_dirty" && message.context) {
...
document.querySelector("#save")!.addEventListener("click", () => {
  ...
  configuredPort = Number(port.value);
  post({ type: "set_port", port: configuredPort });
