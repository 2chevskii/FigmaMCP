...
void loadConfig();
figma.on("currentpagechange", () => figma.ui.postMessage({ type: "context_dirty", context: connectionContext() }));
...
async function loadConfig(): Promise<void> { const value = await figma.clientStorage.getAsync(key); figma.ui.postMessage({ type: "config_loaded", port: typeof value === "number" ? value : 3846, context: connectionContext() }); }
async function handleRequest(bytes: Uint8Array): Promise<void> {
  ...
  try {
    const response: Envelope = { type: "response", protocol_version: PROTOCOL_VERSION, connection_id: request.connection_id, request_id: request.request_id, sent_at: now(), payload: { connection_id: request.connection_id, ...metadata() } };
...
function connectionContext(): Record<string, unknown> { return { plugin_version: "0.1.0", editor_type: figma.editorType, mode: figma.mode, document_name: figma.root.name, current_page: { id: figma.currentPage.id, name: figma.currentPage.name } }; }
