...
public sealed partial class FigmaTools
{
    ...
    [McpServerTool(
        Name = "create_figma_media",
        Title = "Create Figma media",
        Destructive = false,
        Idempotent = false,
        OpenWorld = false
    )]
    ...
    public Task<object> CreateFigmaMedia(
        [Description("The live Figma plugin connection UUID.")] string connection_id,
        [Description("Object with kind=video and data_base64.")] JsonElement input,
        CancellationToken cancellationToken
    ) => InvokeAsync(connection_id, "create_figma_media", input, cancellationToken);

    [McpServerTool(
        Name = "list_figma_shaders",
        Title = "List or import Figma shaders",
        ReadOnly = false,
        Destructive = false,
        Idempotent = true,
        OpenWorld = false
    ...
    [McpServerTool(
        Name = "load_figma_brushes",
        Title = "Load Figma brushes",
        Destructive = false,
        Idempotent = true,
        OpenWorld = false
    )]
    ...
    {
        if (
            !TryReadScreenshotInput(
                input,
                out var nodeId,
                out var scale,
                out var contentsOnly,
                out var validationError
            )
        )
        {
            return ScreenshotError("invalid_argument", validationError, connection_id);
        }

        var settings = new Dictionary<string, object?>
        {
            ["format"] = "PNG",
            ["constraint"] = new { type = "SCALE", value = scale },
        };
        ...
        var exportInput = JsonSerializer.SerializeToElement(
            new { node_ids = new[] { nodeId }, settings }
        );
    ...
    [McpServerTool(
        Name = "encode_figma_binary",
        Title = "Inspect Figma binary",
        ReadOnly = true,
        Destructive = false,
        OpenWorld = false
    )]
    ...
    [McpServerTool(
        Name = "update_figma_prototype",
        Title = "Update Figma prototype",
        Destructive = false,
        Idempotent = true,
        OpenWorld = false
    )]
    ...
    [McpServerTool(
        Name = "get_figma_viewport",
        Title = "Get Figma viewport",
        ReadOnly = true,
        Destructive = false,
        OpenWorld = false
    )]
    ...
    [McpServerTool(
        Name = "set_figma_viewport",
        Title = "Set Figma viewport",
        Destructive = false,
        Idempotent = true,
        OpenWorld = false
    )]
    ...
    [McpServerTool(
        Name = "notify_figma_user",
        Title = "Notify Figma user",
        Destructive = false,
        Idempotent = false,
        OpenWorld = false
    )]
    ...
    [McpServerTool(
        Name = "commit_figma_undo",
        Title = "Control Figma undo",
        Destructive = true,
        Idempotent = false,
        OpenWorld = false
    )]
    ...
    [McpServerTool(
        Name = "save_figma_version",
        Title = "Save Figma version",
        Destructive = false,
        Idempotent = false,
        OpenWorld = false
    )]
    ...
    [McpServerTool(
        Name = "get_figma_file_thumbnail_node",
        Title = "Get Figma file thumbnail node",
        ReadOnly = true,
        Destructive = false,
        OpenWorld = false
    )]
    ...
    [McpServerTool(
        Name = "set_figma_file_thumbnail_node",
        Title = "Set Figma file thumbnail node",
        Destructive = false,
        Idempotent = true,
        OpenWorld = false
    )]
    ...
    )
    {
        ...
        if (
            !input.TryGetProperty("node_id", out var nodeIdElement)
        ...
        {
            if (contentsOnlyElement.ValueKind is not JsonValueKind.True and not JsonValueKind.False)
            {
    ...
    private static CallToolResult ScreenshotError(string code, string message, string connectionId)
    {
        var error = JsonSerializer.SerializeToElement(Error(code, message, connectionId));
        return new CallToolResult
        {
            IsError = true,
            Content = [new TextContentBlock { Text = error.GetRawText() }],
